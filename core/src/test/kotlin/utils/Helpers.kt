...
fun fakeEnv(vars: Map<String, String>) =
    object : Dotenv {
        override fun entries() = emptySet<io.github.cdimascio.dotenv.DotenvEntry>()

        override fun entries(filter: Dotenv.Filter) =
            emptySet<io.github.cdimascio.dotenv.DotenvEntry>()
...
/**
 * Helper to create a crontab manager instance with a temp file. Used in tests only (NOT THREAD
 * SAFE)
 */
